...
/**
 * Used to place the player as far away from the goal node as possible. We start
 * in the corner and have a goal of the opposite corner.
 * 
 * We use the best first traversal algorithm. We keep track of the worst
 * heuristic to the actual in game goal node.
 * 
 * Once complete we place the player at the worst heuristic.
 * 
 * @author Ronan
 */
...
//		this.newGoal = getGoalNode();
		...
		// we want the worst one, that means more nodes expanded and we have a
		// better chance of finding a worse heuristic
		...
		while (!queue.isEmpty()) { // while not empty
			...
			for (int i = 0; i < children.length; i++) { // iterate through
				...
				if (children[i] != null && !children[i].isVisited()) { // if not
					...
					// queue.addLast(children[i]); // add child to back of list
					// (this is actually twice as fast, but that depends where
					// you are in the maze)
				}
			}
			...
			// Swapped
			Collections.sort(queue, (Node node1, Node node2) -> node1.getHeuristic(goal) - node2.getHeuristic(goal)); // normal:
																														// 153
			// Collections.sort(queue, (Node node1, Node node2) ->
			// node2.getHeuristic(goal) - node1.getHeuristic(goal)); // swapped:
			// 17596 // Worst First

			// +
			// Collections.sort(queue, (Node node1, Node node2) ->
			// node1.getHeuristic(goal) + node2.getHeuristic(goal)); // +: 3916
			// Collections.sort(queue, (Node node1, Node node2) ->
			// node2.getHeuristic(goal) + node1.getHeuristic(goal)); // +
			// swapped: 1267
